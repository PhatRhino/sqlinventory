...
USING (
    SELECT
        ...
        , (SELECT value_in_use FROM sys.configurations WHERE name = 'max server memory (MB)') * 1024 AS MaxMemory
        , (SELECT value_in_use FROM sys.configurations WHERE name = 'min server memory (MB)') * 1024 AS MinMemory
        ...
        , (SELECT TOP 1 HostID FROM HostInfo WHERE Hostname = @@SERVERNAME) AS HostID
) AS source
...
WHEN NOT MATCHED THEN
    ...
    VALUES (
        ...
        , source.Level
        , source.UpdateLevel
        , source.VersionDECLARE @DefaultBackupDirectory NVARCHAR(500);
